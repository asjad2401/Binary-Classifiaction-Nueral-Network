#run this file to create a trainset.h5 file from images 
...
labels =[] #labels here 0 or 1 
for path in paths:
    images.append(cv2.imread(path))
    
imagedata =  np.concatenate([arr[np.newaxis, ...] for arr in images], axis=0)

with h5py.File('trainset.h5','w') as f:
    f.create_dataset('train_set_x', data=imagedata)
